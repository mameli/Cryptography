...
from modInverse import modinv, minor, inverseMatrixMod26
...
def detNotNUllSub(matrix, m):
    ...
    for x in permutations(columns, r = m):
        ...
        det = np.rint(np.linalg.det(matrix[:,listCol]) % 26).astype(int)
...
def decrypt(ct,key):
    m = int(math.sqrt(len(key)))
    matKey = charToMatrix(key, m)
    detModulo26 = modinv(np.rint(np.linalg.det(matKey) % 26).astype(int), 26)
    matKeyInv = np.rint(detModulo26 * inv(matKey) * np.linalg.det(matKey) % 26).astype(int)
...
def cryptAnalysis(pt, ct, m):
    ...
    if colInd == None:
        return None
    detModulo26 = modinv(np.rint(np.linalg.det(matPt[:,colInd]) % 26).astype(int), 26)
    # matPtInv = inverseMatrixMod26(matPt[:,colInd], m)
    matPtInv = np.rint(detModulo26 * inv(matPt[:,colInd]) * np.linalg.det(matPt[:,colInd]) % 26).astype(int)
